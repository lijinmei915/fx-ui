...
export function TokensMotionPage({ actions, lang, motionTokens }: Props) {
  ...
  return <div className={docsSpacing.pageStack}>
    <section id="tokens-motion" className="flex flex-col gap-2"><PageLead crumb={lang === "en" ? "Design Tokens / Motion" : "设计令牌 / 动效"} title={lang === "en" ? "Motion" : "动效"} lead={lang === "en" ? "Motion tokens define shared durations and easing curves; framework adapters map them to component enter, exit, and state transitions." : "动效 Token 统一定义时长与缓动曲线；各框架适配层再将其映射到组件的进入、退出和状态过渡。"} actions={actions} /></section>
