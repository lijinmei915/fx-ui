...
主题面板的字号基准同时代表文档站外壳密度：`12px` 是紧凑、`14px` 是标准、`16px` 是宽松。标准档按主流后台文档站观感收敛：顶栏 `56px`、左侧栏 `240px`、搜索框和工具栏按钮 `28–32px` 热区。主导航和侧栏入口使用独立菜单字号：12 基准显示 14px，14/16 基准显示 16px，不继续随宽松档放大；紧凑和宽松只在这个基础上调整高度、内边距和分组间距，不改变页面信息层级。
...
- **不放第二行说明**：补充信息下沉到头部之后的正文，不堆在头部组里。
- **不在头部组内另加横线**：`PageLead` 不渲染分隔线；头部下方直接进入正文区块。
- **内部取值固定**：面包屑 `text-sm` / 400 / 弱文本色，当前页 500 / 主文本色；标题 `text-3xl` / 700；说明 `text-base` / 400 / 弱文本色；整体随界面字号基准联动。
- **内部间距固定**：面包屑→标题 `mb-3`；标题→说明 `mt-2`；标题组与右侧操作桌面端顶对齐。
- **面包屑来源固定**：`crumb = 导航分组 / 当前页名称`，必须和左侧导航分组与当前页 label 保持一致；最后一段自动识别为当前页。
...
- **第二句说明的规则**：默认不写。只有首句无法交代清楚页面边界、且这条补充会直接影响读者判断或选型时，才允许加第二句；像「支持多种尺寸/状态/视觉变体」这类从下方总览、场景或 API 一眼就能看出的泛化描述，一律视为废话，删除不写。

### 模块说明与行高

- **每个模块（h2 区块）都要有标题 + 一句说明**，说明只回答"这个模块是干嘛的"，尽量一句话。
- **模块说明字号固定**：`text-2xl` 模块标题下的说明统一用 `docsSpacing.sectionDesc`（= `text-base text-muted-foreground`），所有页一致。
- **说明不与下方内容重复**：表格 / 场景列 / 卡片里已经逐项写明的，说明里不再罗列（如圆角档位的逐档→组件映射在场景列，说明就别再抄一遍）。
- **行高不手写**：正文/说明的行高由 Tailwind `text-*` 变量自带。不要再加 `leading-7`/`leading-8` 把行距抬到 2.0+，那样换行太散、不符合主流正文行高（约 1.5）。
- **组件页模块说明的固定文案**：组件文档页（含 Button 独立页与 `StandardDocPage`）的标准模块说明统一口径，不要一页一个写法：
  - 头部一律用 `PageLead`（面包屑 `组件 / Xxx` + 大标题 + 一句 lead「这个组件干嘛的」），不要再写裸 `<h1>`。
...
- **示例列宽**：场景表「示例」列容器统一 `max-w-[400px]`——不到则自适应内容宽度，超过才约束并让内部 `flex-wrap` 折行（示例多/宽时不撑爆表格）。
