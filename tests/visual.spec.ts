...
const playgroundManifest = JSON.parse(
  ...
  customPlaygrounds?: Record<
    string,
    {
      stories?: unknown[];
      visual?: { route: string; selector: string; screenshot?: string };
    }
  >;
  components?: Record<
    string,
  ...
  autoVisuals?: Record<
    string,
    { route: string; selector: string; test: string }
  >;
  pageVisuals?: Record<
    string,
    { route: string; selector: string; test: string }
  >;
  baselineVisuals?: Record<
    string,
    { route: string; selector: string; test: string }
  >;
...
function storyParameter(pointer: string, key: "intent") {
  ...
  const entry =
    group === "customPlaygrounds"
      ? playgroundManifest.customPlaygrounds?.[id]
      : playgroundManifest.components?.[id];
  const story = entry?.stories?.[0] as
...
test("网站卡片容器", async ({ page }) => {
  ...
  await page.goto(`/${visual.route}`);
  const card = page
    .locator("[data-website-card-container]")
    .filter({ hasText: "内部区域" });
...
test("网站卡片容器跟随全局阴影", async ({ page }) => {
  await page.addInitScript(() => {
    window.localStorage.setItem(
      "fx-ui-theme-config",
      JSON.stringify({ shadowLevel: "none" }),
    );
  });
...
test("Icon 预览", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#components.icon"]',
    ),
  ).toHaveCount(1);
...
test("客户列表页模板", async ({ page }) => {
  ...
  await page.goto(`/${visual.route}`);
  const frame = page
    .locator("#template-customer-list")
...
test("客户列表视觉校准台", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 });
  const visual = visualConfig("baselineVisuals", "customer-list-calibration");
  await page.goto(`/${visual.route}`);
  const calibration = page.locator(visual.selector);
  await expect(calibration).toBeVisible();
  await expect(
    calibration.locator('[data-slot="component-playground"]'),
  ).toHaveCount(1);
  await expect(calibration).toHaveScreenshot("customer-list-calibration.png", {
    mask: [page.locator('[data-slot="avatar"]')],
  });
});

test("页面搭建器客户列表 MVP", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 });
  const visual = visualConfig("baselineVisuals", "page-builder");
  await page.goto(`/${visual.route}`);
  const builder = page.locator(visual.selector);
  await expect(builder).toBeVisible();
  await expect(builder.locator('[data-slot="page-builder"]')).toHaveCount(1);
  await expect(builder).toHaveScreenshot("page-builder.png", {
    mask: [page.locator('[data-slot="avatar"]')],
  });
});

test("页面搭建器空白页", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 });
  const visual = visualConfig("baselineVisuals", "page-builder-blank");
  await page.goto(`/${visual.route}`);
  const builder = page.locator(visual.selector);
  await builder
    ...
    .click();
  await expect(
    builder.locator('[data-slot="page-builder-preview"]'),
  ).toContainText("从空白页开始");
  await expect(builder).toHaveScreenshot("page-builder-blank.png");
});

test("组件搭建器 MVP", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 });
  const visual = visualConfig("baselineVisuals", "component-builder");
  await page.goto(`/${visual.route}`);
  const builder = page.locator(visual.selector);
...
test("业务组件属性契约搭建器", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 });
  const visual = visualConfig("baselineVisuals", "business-component-builder");
  await page.goto(`/${visual.route}`);
  const builder = page.locator(visual.selector);
  await builder.getByLabel("搭建模式").click();
  ...
  await composition.locator('[data-node-type="button"]').click();
  await composition.getByLabel("内容属性").fill("提交");
  await composition
  ...
  await expect(
    composition
      .locator('[data-slot="business-component-preview"]')
      .locator('[data-slot="select-trigger"]'),
  ).toBeVisible();
  await expect(builder).toHaveScreenshot("business-component-builder.png");
});
...
test("客户简报报告", async ({ page }) => {
  ...
  await expect(cards.first()).toHaveScreenshot(
    "customer-briefing-profile.png",
    {
      mask: [page.locator('[data-slot="avatar"]')],
    },
  );
  await expect(cards.last()).toHaveScreenshot(
    "customer-briefing-risk-actions.png",
    {
      mask: [page.locator('[data-slot="avatar"]')],
    },
  );
...
test("Button 交互调试台", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#customPlaygrounds.button"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator('[data-slot="component-playground-stories"]'),
  ).toHaveCount(0);
  await expect(
    pg.locator(
      `[data-story-count="${storyCount("customPlaygrounds.button")}"]`,
    ),
  ).toHaveCount(1);
...
test("Tag 交互调试台", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#components.tag"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator('[data-slot="component-playground-stories"]'),
  ...
  await expect(
    pg.locator(`[data-story-count="${storyCount("components.tag")}"]`),
  ).toHaveCount(1);
  await expect(pg).toHaveScreenshot(visual.screenshot);
});
...
test("Badge 交互调试台", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#components.badge"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator('[data-slot="component-playground-stories"]'),
  ...
  await expect(
    pg.locator(`[data-story-count="${storyCount("components.badge")}"]`),
  ).toHaveCount(1);
  await expect(pg).toHaveScreenshot(visual.screenshot);
});
...
test("Tooltip Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.tooltip"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.tooltip")}"]`),
  ).toHaveCount(1);
});
...
test("Avatar Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#components.avatar"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator('[data-slot="component-playground-stories"]'),
  ...
  await expect(
    pg.locator(`[data-story-count="${storyCount("components.avatar")}"]`),
  ).toHaveCount(1);
  ...
  const selectTextExample = async (storyName: string) => {
    ...
    const text = contentControl.getByRole("button", {
      name: "文字",
      exact: true,
    });
    await text.click();
...
test("Breadcrumb Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.breadcrumb"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.breadcrumb")}"]`),
  ).toHaveCount(1);
});
...
test("Field Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.field")}"]`),
  ).toHaveCount(1);
...
test("Label Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.label")}"]`),
  ).toHaveCount(1);
...
test("Table Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#components.table"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("components.table")}"]`),
  ).toHaveCount(1);
...
test("Table 成熟能力矩阵", async ({ page }) => {
  ...
  await stories
    .getByRole("button", { name: "基础数据表", exact: true })
    .click();
  await expect(
    playground.locator('[data-slot="table-container"]').first(),
  ).toHaveScreenshot("table-plain-default.png");
  ...
  await expect(
    playground.locator('[data-slot="table-container"]').first(),
  ).toHaveScreenshot("table-bordered-compact-business.png");

  await stories
    .getByRole("button", { name: "斑马纹汇总表", exact: true })
    .click();
  await expect(
    playground.locator('[data-slot="table-container"]').first(),
  ).toHaveScreenshot("table-striped-comfortable-summary.png");
});
...
test("AlertDialog Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.alert-dialog"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(
      `[data-story-count="${storyCount("autoStories.alert-dialog")}"]`,
    ),
  ).toHaveCount(1);
});
...
test("Sheet Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.sheet"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.sheet")}"]`),
  ).toHaveCount(1);
});
...
test("Dialog / Sheet 成熟尺寸矩阵", async ({ page }) => {
  ...
  const dialogStories = dialogPlayground.locator(
    '[data-slot="component-playground-stories"]',
  );
  await dialogStories
    .getByRole("button", { name: "确认弹窗", exact: true })
    .click();
  await dialogPlayground
    .getByRole("button", { name: "发布版本", exact: true })
    .click();
  await expect(page.getByRole("dialog")).toHaveScreenshot(
    "dialog-confirm-sm.png",
  );
  await page.keyboard.press("Escape");
  await dialogStories
    .getByRole("button", { name: "宽版审阅弹窗", exact: true })
    .click();
  await dialogPlayground
    .getByRole("button", { name: "审阅发布内容", exact: true })
    .click();
  await expect(page.getByRole("dialog")).toHaveScreenshot(
    "dialog-review-lg.png",
  );
  await page.keyboard.press("Escape");
  ...
  const sheetStories = sheetPlayground.locator(
    '[data-slot="component-playground-stories"]',
  );
  await sheetStories
    .getByRole("button", { name: "宽版高级配置", exact: true })
    .click();
  await sheetPlayground
    .getByRole("button", { name: "高级配置", exact: true })
    .click();
...
test("Skeleton Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.skeleton"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.skeleton")}"]`),
  ).toHaveCount(1);
});
...
test("Pagination Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.pagination"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.pagination")}"]`),
  ).toHaveCount(1);
});
...
test("Card Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#components.card"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator('[data-slot="component-playground-stories"]'),
  ...
  await expect(
    pg.locator(`[data-story-count="${storyCount("components.card")}"]`),
  ).toHaveCount(1);
...
test("Card 成熟能力矩阵", async ({ page }) => {
  ...
  await expect(
    playground.locator('[data-slot="card"]').first(),
  ).toHaveScreenshot("card-outline-md.png");

  await stories.getByRole("button", { name: "信息说明", exact: true }).click();
  await expect(
    playground.locator('[data-slot="card"]').first(),
  ).toHaveScreenshot("card-subtle-sm.png");

  await stories.getByRole("button", { name: "可操作项", exact: true }).click();
  await expect(
    playground.locator('[data-slot="card"]').first(),
  ).toHaveScreenshot("card-elevated-lg.png");

  await stories.getByRole("button", { name: "媒体内容", exact: true }).click();
  await expect(
    playground.locator('[data-slot="card"]').first(),
  ).toHaveScreenshot("card-media-outline-md.png");
});

test("Checkbox Playground 主入口", async ({ page }) => {
  const visual = visualConfig("customPlaygrounds", "checkbox");
  await page.goto(`/${visual.route}`);
  const pg = page.locator(visual.selector);
  await expect(pg).toBeVisible();
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#customPlaygrounds.checkbox"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator('[data-slot="component-playground-stories"]'),
  ).toHaveCount(0);
  await expect(pg).toHaveScreenshot("checkbox-playground-default-14.png");

  await pg.getByRole("button", { name: "复选组", exact: true }).click();
  await pg.getByRole("button", { name: "小12", exact: true }).click();
  await pg.getByRole("button", { name: "横向", exact: true }).click();
  await expect(pg).toHaveScreenshot("checkbox-playground-sm-horizontal.png");

  await pg.getByRole("button", { name: "已选禁用", exact: true }).click();
  await expect(pg).toHaveScreenshot(
    "checkbox-playground-sm-checked-disabled.png",
  );
});

test("Switch Playground 主入口", async ({ page }) => {
  const visual = visualConfig("customPlaygrounds", "switch");
  await page.goto(`/${visual.route}`);
  const pg = page.locator(visual.selector);
  await expect(pg).toBeVisible();
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#customPlaygrounds.switch"]',
    ),
  ).toHaveCount(1);
  await expect(pg).toHaveScreenshot("switch-playground-default.png");

  await pg.getByRole("button", { name: "文字", exact: true }).click();
  await pg.getByRole("button", { name: "开", exact: true }).click();
  await expect(pg).toHaveScreenshot("switch-playground-checked-content.png");

  await pg.getByRole("button", { name: "加载", exact: true }).click();
  await expect(pg).toHaveScreenshot("switch-playground-loading.png");

  await pg.getByRole("button", { name: "默认", exact: true }).nth(1).click();
  await pg.getByRole("button", { name: "图标", exact: true }).click();
  await pg.getByRole("button", { name: "中32", exact: true }).click();
  await expect(pg).toHaveScreenshot("switch-playground-icon-medium.png");
});
...
test("Textarea Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.textarea"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.textarea")}"]`),
  ).toHaveCount(1);
});

test("Signature Playground 主入口", async ({ page }) => {
  const visual = visualConfig("pageVisuals", "signature");
  await page.goto(`/${visual.route}`);
  const pg = page.locator(visual.selector);
  await expect(pg).toBeVisible();
  await expect(pg.locator('[data-slot="signature"]')).toBeVisible();
  await expect(pg.locator('[data-slot="signature-canvas"]')).toBeVisible();
  await expect(pg.locator('[data-slot="signature-clear"]')).toBeVisible();
  await expect(pg).toHaveScreenshot("signature-playground.png");
});
...
test("Separator Playground 主入口", async ({ page }) => {
  ...
  await expect(
    page.locator(
      'a[href="#separator-overview"], a[href="#separator-preview"], a[href="#separator-usage"]',
    ),
  ).toHaveCount(0);
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#components.separator"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator('[data-slot="component-playground-stories"]'),
  ).toHaveCount(0);
  const orientationControl = pg
    .locator("label")
    .filter({ hasText: "方向" })
    .locator("..");
  await expect(
    orientationControl.getByRole("button", { name: "水平", exact: true }),
  ).toHaveCount(1);
  await expect(
    orientationControl.getByRole("button", { name: "垂直", exact: true }),
  ).toHaveCount(1);
  await expect(pg).toHaveScreenshot("separator-playground.png");
});

test("Slider Playground 主入口", async ({ page }) => {
  const visual = visualConfig("customPlaygrounds", "slider");
  await page.goto(`/${visual.route}`);
  const pg = page.locator(visual.selector);
  await expect(pg).toBeVisible();
  await expect(page.locator("#slider-overview")).toHaveCount(0);
  await expect(page.locator("#slider-preview")).toHaveCount(0);
  await expect(page.locator("#slider-usage")).toHaveCount(0);
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#customPlaygrounds.slider"]',
    ),
  ).toHaveCount(1);
  await expect(pg).toHaveScreenshot("slider-playground-single.png");

  await pg.getByRole("button", { name: "范围", exact: true }).click();
  await expect(pg).toHaveScreenshot("slider-playground-range.png");
});

test("Upload Playground 主入口", async ({ page }) => {
  const visual = visualConfig("components", "upload");
  await page.goto(`/${visual.route}`);
  const pg = page.locator(visual.selector);
  await expect(pg).toBeVisible();
  await expect(page.locator("#upload-overview")).toHaveCount(0);
  await expect(page.locator("#upload-preview")).toHaveCount(0);
  await expect(page.locator("#upload-usage")).toHaveCount(0);
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#components.upload"]',
    ),
  ).toHaveCount(1);
  await expect(pg).toHaveScreenshot(visual.screenshot);
  await pg.getByRole("button", { name: "拖拽", exact: true }).click();
  await expect(pg.locator('[data-slot="upload"]').first()).toHaveScreenshot(
    "upload-dropzone.png",
  );
});

test("IconPicker Playground 主入口", async ({ page }) => {
  const visual = visualConfig("customPlaygrounds", "iconPicker");
  await page.goto(`/${visual.route}`);
  const pg = page.locator(visual.selector);
  await expect(pg).toBeVisible();
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#customPlaygrounds.iconPicker"]',
    ),
  ).toHaveCount(1);
  await expect(pg).toHaveScreenshot(visual.screenshot);
  await expect(
    pg.locator('[data-slot="icon-picker"]').first(),
  ).toHaveScreenshot("icon-picker-surface.png");

  await pg.getByRole("button", { name: "无结果", exact: true }).click();
  await expect(
    pg.locator('[data-slot="icon-picker"]').first(),
  ).toHaveScreenshot("icon-picker-empty.png");

  await pg.getByRole("button", { name: "加载中", exact: true }).click();
  await expect(
    pg.locator('[data-slot="icon-picker"]').first(),
  ).toHaveScreenshot("icon-picker-loading.png");

  await pg.getByRole("button", { name: "加载失败", exact: true }).click();
  await expect(
    pg.locator('[data-slot="icon-picker"]').first(),
  ).toHaveScreenshot("icon-picker-error.png");

  await pg.getByRole("button", { name: "上传", exact: true }).click();
  await expect(
    pg.locator('[data-slot="icon-picker"]').first(),
  ).toHaveScreenshot("icon-picker-upload.png");
});

test("ColorPicker Playground 主入口", async ({ page }) => {
  const visual = visualConfig("customPlaygrounds", "colorPicker");
  await page.goto(`/${visual.route}`);
  const pg = page.locator(visual.selector);
  await expect(pg).toBeVisible();
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#customPlaygrounds.colorPicker"]',
    ),
  ).toHaveCount(1);
  const panel = page.locator('[data-slot="color-picker"]');
  await expect(panel).toBeVisible();
  await expect(pg).toHaveScreenshot(visual.screenshot);
  await expect(panel.locator("..")).toHaveScreenshot("color-picker-panel.png");
});

test("Transfer Playground 主入口", async ({ page }) => {
  const visual = visualConfig("customPlaygrounds", "transfer");
  await page.goto(`/${visual.route}`);
  const pg = page.locator(visual.selector);
  await expect(pg).toBeVisible();
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#customPlaygrounds.transfer"]',
    ),
  ).toHaveCount(1);
  await expect(pg).toHaveScreenshot(visual.screenshot);

  await pg.getByRole("button", { name: "开启", exact: true }).click();
  await expect(pg.locator('[data-slot="transfer"]').first()).toHaveScreenshot(
    "transfer-search.png",
  );

  await pg.getByRole("button", { name: "是", exact: true }).last().click();
  await expect(pg.locator('[data-slot="transfer"]').first()).toHaveScreenshot(
    "transfer-loading.png",
  );

  await pg.getByRole("button", { name: "否", exact: true }).last().click();
  await pg.getByRole("button", { name: "错误", exact: true }).click();
  await expect(pg.locator('[data-slot="transfer"]').first()).toHaveScreenshot(
    "transfer-error.png",
  );
});

test("ConditionBuilder Playground 主入口", async ({ page }) => {
  const visual = visualConfig("customPlaygrounds", "conditionBuilder");
  await page.goto(`/${visual.route}`);
  const pg = page.locator(visual.selector);
  const builder = pg.locator('[data-slot="condition-builder"]').first();
  await expect(pg).toBeVisible();
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#customPlaygrounds.conditionBuilder"]',
    ),
  ).toHaveCount(1);
  await expect(pg).toHaveScreenshot(visual.screenshot);

  await pg.getByRole("button", { name: "多组 OR", exact: true }).click();
  await expect(builder).toHaveScreenshot("condition-builder-or-groups.png");

  await pg.getByRole("button", { name: "是", exact: true }).last().click();
  await expect(builder).toHaveScreenshot("condition-builder-read-only.png");
});
...
test("Spinner Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.spinner"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.spinner")}"]`),
  ).toHaveCount(1);
...
test("Collapsible Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.collapsible"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.collapsible")}"]`),
  ).toHaveCount(1);
...
test("Tabs Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.tabs"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.tabs")}"]`),
  ).toHaveCount(1);
  await expect(pg).toHaveScreenshot("tabs-playground.png");
  const stories = pg.locator('[data-slot="component-playground-stories"]');
  await stories
    .getByRole("button", { name: "默认样式切页", exact: true })
    .click();
  await expect(pg.locator('[data-slot="tabs"]').first()).toHaveScreenshot(
    "tabs-default-md.png",
  );
  await stories
    .getByRole("button", { name: "下划线样式", exact: true })
    .click();
  await expect(pg.locator('[data-slot="tabs"]').first()).toHaveScreenshot(
    "tabs-line-sm.png",
  );
...
test("Popover Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.popover"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.popover")}"]`),
  ).toHaveCount(1);
...
test("Popover 成熟尺寸矩阵", async ({ page }) => {
  ...
  await stories
    .getByRole("button", { name: "信息说明卡", exact: true })
    .click();
  ...
  await playground
    .getByRole("button", { name: "筛选条件", exact: true })
    .click();
  await expect(page.locator('[data-slot="popover-content"]')).toHaveScreenshot(
    "popover-filter-lg.png",
  );
});
...
test("Sidebar Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.sidebar"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.sidebar")}"]`),
  ).toHaveCount(1);
...
test("Command Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.command"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.command")}"]`),
  ).toHaveCount(1);
...
test("ToggleGroup Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.toggle-group"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(
      `[data-story-count="${storyCount("autoStories.toggle-group")}"]`,
    ),
  ).toHaveCount(1);
...
test("Toast Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.toast"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(`[data-story-count="${storyCount("autoStories.toast")}"]`),
  ).toHaveCount(1);
  await expect(pg).toHaveScreenshot("toast-playground.png");
});

test("RadioGroup Playground 主入口", async ({ page }) => {
  const visual = visualConfig("customPlaygrounds", "radioGroup");
  await page.goto(`/${visual.route}`);
  const pg = page.locator(visual.selector);
  await expect(pg).toBeVisible();
  await expect(page.locator("#radio-group-overview")).toHaveCount(0);
  await expect(page.locator("#radio-group-preview")).toHaveCount(0);
  await expect(page.locator("#radio-group-usage")).toHaveCount(0);
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#customPlaygrounds.radioGroup"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator('[data-slot="component-playground-stories"]'),
  ).toHaveCount(0);
  await expect(pg).toHaveScreenshot("radio-group-playground-default-14.png");

  await pg.getByRole("button", { name: "小12", exact: true }).click();
  await pg.getByRole("button", { name: "横向", exact: true }).click();
  await expect(pg).toHaveScreenshot("radio-group-playground-sm-horizontal.png");

  await pg.getByRole("button", { name: "已选禁用", exact: true }).click();
  await expect(pg).toHaveScreenshot(
    "radio-group-playground-sm-checked-disabled.png",
  );
});
...
test("DropdownMenu Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#autoStories.dropdown-menu"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator(
      `[data-story-count="${storyCount("autoStories.dropdown-menu")}"]`,
    ),
  ).toHaveCount(1);
...
test("DropdownMenu 真实键盘菜单", async ({ page }) => {
  ...
  await expect(page.getByRole("menu").first()).toHaveScreenshot(
    "dropdown-menu-submenu-root.png",
  );
});
...
test("Link Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#components.link"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator('[data-slot="component-playground-stories"]'),
...
test("ButtonGroup Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#components.buttonGroup"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator('[data-slot="component-playground-stories"]'),
  ...
  await expect(
    pg.locator(`[data-story-count="${storyCount("components.buttonGroup")}"]`),
  ).toHaveCount(1);
  await expect(pg).toHaveScreenshot(visual.screenshot);
});
...
test("Input 组件制作台", async ({ page }) => {
  ...
  await expect(
    workbench.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#components.input"]',
    ),
  ).toHaveCount(1);
  await expect(
    workbench.locator('[data-slot="component-playground-stories"]'),
  ).toHaveCount(0);
  await expect(workbench.getByRole("treeitem")).toHaveCount(0);
  await expect(workbench.locator("label")).toHaveText([
    "占位文字",
    "前置内容",
    "后置内容",
    "尺寸",
  ...
  const leadingControl = workbench
    .locator("label")
    .filter({ hasText: "前置内容" })
    .locator("..");
  const trailingControl = workbench
    .locator("label")
    .filter({ hasText: "后置内容" })
    .locator("..");
  await leadingControl
    .getByRole("button", { name: "符号", exact: true })
    .click();
  await trailingControl
    .getByRole("button", { name: "符号", exact: true })
    .click();
  ...
  await expect(
    workbench.locator('[data-slot="input-workbench-preview"]'),
  ).toContainText("%");
  await leadingControl.getByRole("button", { name: "无", exact: true }).click();
  await trailingControl
    .getByRole("button", { name: "无", exact: true })
    .click();
  await leadingControl
    .getByRole("button", { name: "标签", exact: true })
    .click();
  const inputGroup = workbench.locator(
    '[data-slot="input-workbench-preview"] [data-slot="input-group"]',
  );
  const inputAddon = workbench.locator(
    '[data-slot="input-workbench-preview"] [data-slot="input-addon"]',
  );
  await expect(inputAddon).toHaveCount(1);
  const groupBackground = await inputGroup.evaluate(
    (element) => getComputedStyle(element).backgroundColor,
  );
  const addonBackground = await inputAddon.evaluate(
    (element) => getComputedStyle(element).backgroundColor,
  );
  const groupBorder = await inputGroup.evaluate(
    (element) => getComputedStyle(element).borderColor,
  );
  const addonDivider = await inputAddon.evaluate(
  ...
  await workbench
    ...
    .click();
  const searchInput = workbench.locator(
    '[data-slot="input-workbench-preview"] [data-slot="input"]',
  );
  await expect(searchInput).toHaveAttribute("type", "search");
  await expect(
    leadingControl.getByRole("button", { name: "搜索图标", exact: true }),
  ).toBeVisible();
  await expect(
    workbench.locator(
      '[data-slot="input-workbench-preview"] [data-slot="input-affix"]',
    ),
  ).toHaveCount(1);
  await expect(
    leadingControl.getByRole("button", { name: "邮箱图标", exact: true }),
  ).toHaveCount(0);
  await expect(
    trailingControl.getByRole("button", { name: "显示密码", exact: true }),
  ).toHaveCount(0);
  await trailingControl
    .getByRole("button", { name: "搜索图标", exact: true })
    .click();
  const searchAffix = workbench.locator(
    '[data-slot="input-workbench-preview"] [data-slot="input-affix"]',
  );
  ...
  await expect(
    workbench.locator(
      '[data-slot="input-workbench-preview"] [data-slot="input-action"]',
    ),
  ).toHaveCount(0);
  await searchInput.fill("客户");
  const clearSearch = workbench.locator(
    '[data-slot="input-workbench-preview"] [data-slot="input-action"]',
  );
  ...
  await expect(
    trailingControl.getByRole("button", { name: "清除", exact: true }),
  ).toHaveCount(0);
  await trailingControl
    .getByRole("button", { name: "无", exact: true })
    .click();
  await workbench
    ...
    .getByRole("button", { name: "文本", exact: true })
    .click();
  const workbenchCard = workbench.locator(
    ':scope > [data-slot="component-playground"]',
  );
  ...
  await workbench
    ...
    .filter({ hasText: "输入类型" })
    .locator("..")
    .getByRole("button", { name: "数字", exact: true })
    .click();
  const numberInput = workbench.locator(
    '[data-slot="input-workbench-preview"] [data-slot="input"]',
  );
  ...
  await workbench
    ...
    .click();
  await trailingControl
    .getByRole("button", { name: "主搜索", exact: true })
    .click();
  const primaryAction = workbench.locator(
    '[data-slot="input-workbench-preview"] [data-slot="input-action"]',
  );
  ...
  await expect(
    workbench.locator('[data-slot="input-workbench-preview"]'),
  ).toHaveScreenshot("input-primary-action.png");
  await trailingControl
    .getByRole("button", { name: "无", exact: true })
    .click();
  await workbench
    ...
    .filter({ hasText: "输入类型" })
    .locator("..")
    .getByRole("button", { name: "邮箱", exact: true })
    .click();
  await expect(
    leadingControl.getByRole("button", { name: "搜索图标", exact: true }),
  ).toHaveCount(0);
  ...
  const emailInput = workbench.locator(
    '[data-slot="input-workbench-preview"] [data-slot="input"]',
  );
  ...
  await expect(
    workbench.getByText("请输入有效的邮箱地址。", { exact: true }),
  ).toHaveCount(0);
  ...
  await expect(
    workbench.getByRole("button", { name: "校验格式", exact: true }),
  ).toHaveCount(0);
  await workbench
    .getByRole("button", { name: "编辑组件", exact: true })
    .click();
  const fieldControl = workbench
    .locator("label")
    .filter({ hasText: "字段包装" })
    .locator("..");
  await fieldControl.getByRole("button", { name: "字段", exact: true }).click();
  const wrappedEmailInput = workbench.locator(
    '[data-slot="input-workbench-preview"] [data-slot="input"]',
  );
  ...
  await workbench
    ...
    .filter({ hasText: "输入类型" })
    .locator("..")
    .getByRole("button", { name: "密码", exact: true })
    .click();
  const passwordInput = workbench.locator(
    '[data-slot="input-workbench-preview"] [data-slot="input"]',
  );
  await expect(passwordInput).toHaveAttribute("type", "password");
  const passwordToggle = workbench.locator(
    '[data-slot="input-workbench-preview"] [data-slot="input-action"]',
  );
  ...
  const editingLeadingControl = workbench
    .locator("label")
    .filter({ hasText: "前置内容" })
    .locator("..");
  await editingLeadingControl
    .getByRole("button", { name: "标签", exact: true })
    .click();
  ...
  const stateAssignments = workbench.locator(
    '[data-slot="component-playground-state-assignments"]',
  );
  ...
  const normalTokenColor = await normalAssignment
    .locator('[aria-hidden="true"]')
    .evaluate((element) => getComputedStyle(element).backgroundColor);
  const normalBorder = await preview
    .locator('[data-slot="input-group"]')
    .evaluate((element) => getComputedStyle(element).borderColor);
  expect(normalBorder).toBe(normalTokenColor);
  const invalidAssignment = stateAssignments.getByRole("button", {
    name: /错误.*边框.*destructive/,
  });
  const invalidTokenColor = await invalidAssignment
    .locator('[aria-hidden="true"]')
    .evaluate((element) => getComputedStyle(element).backgroundColor);
  await invalidAssignment.click();
  await expect(preview.locator('[data-slot="input"]')).toHaveAttribute(
    "aria-invalid",
    "true",
  );
  await expect(preview.locator('[data-slot="input-group"]')).toHaveCSS(
    "border-color",
    invalidTokenColor,
  );
  await expect(
    workbench.locator('[data-slot="component-playground-validation"]'),
  ).toContainText(`border: ${invalidTokenColor}`);
  const invalidBorder = await preview
    .locator('[data-slot="input-group"]')
    .evaluate((element) => getComputedStyle(element).borderColor);
  ...
  await expect(
    workbench.getByPlaceholder("请输入", { exact: true }),
  ).toBeVisible();
  await expect(preview.locator('[data-slot="input"]')).not.toHaveAttribute(
    "aria-invalid",
    "true",
  );
  await expect(workbench.locator("label")).toHaveText([
...
test("DatePicker Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#customPlaygrounds.datePicker"]',
    ),
  ).toHaveCount(1);
  await expect(
    pg.locator('[data-slot="component-playground-stories"]'),
...
test("DateTimePicker Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#customPlaygrounds.dateTimePicker"]',
    ),
  ).toHaveCount(1);
  await expect(pg).toHaveScreenshot(visual.screenshot);
  ...
  const rangeScenario = rangePg.getByRole("button", {
    name: "日期时间范围",
    exact: true,
  });
  await rangeScenario.click();
  const rangePicker = rangePg.locator('[data-slot="date-time-picker"]');
  await expect(rangePicker).toHaveScreenshot(
    "date-time-picker-range-preview.png",
  );
  ...
  await expect(rangeContent).toHaveScreenshot(
    "date-time-picker-range-panel.png",
  );

  await page.setViewportSize({ width: 390, height: 844 });
  await expect(rangeContent).toBeVisible();
  await expect(rangeContent).toHaveScreenshot(
    "date-time-picker-range-panel-mobile.png",
  );
});
...
test("TimePicker Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#customPlaygrounds.timePicker"]',
    ),
  ).toHaveCount(1);
  await expect(pg.locator("[data-story-count]")).toHaveCount(0);
...
test("PeoplePicker Playground 主入口", async ({ page }) => {
  const visual = visualConfig("customPlaygrounds", "peoplePicker");
  await page.goto(`/${visual.route}`);
  const pg = page.locator(visual.selector);
  await expect(pg).toBeVisible();
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#customPlaygrounds.peoplePicker"]',
    ),
  ).toHaveCount(1);
  const picker = pg.locator('[data-slot="people-picker"]');
  await expect(picker).toHaveAttribute("data-size", "normal");
  await expect(pg).toHaveScreenshot(visual.screenshot);
  await expect(picker).toHaveScreenshot("people-picker-normal.png");
});
...
test("Combobox Playground 主入口", async ({ page }) => {
  const visual = visualConfig("pageVisuals", "combobox");
  await page.goto(`/${visual.route}`);
  const pg = page.locator(visual.selector);
  await expect(pg).toBeVisible();
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#components.combobox"]',
    ),
  ).toHaveCount(1);
  await expect(pg.locator("[data-story-count]")).toHaveCount(0);
  await expect(pg).toHaveScreenshot("combobox-playground.png");
});
...
test("Select Playground 主入口", async ({ page }) => {
  ...
  await expect(
    pg.locator(
      '[data-story-source="docs/data/component-playgrounds.manifest.json#components.select"]',
    ),
  ).toHaveCount(1);
  await expect(pg.locator("[data-story-count]")).toHaveCount(0);
});
...
test("Layout 页面模块", async ({ page }) => {
  ...
  await expect(
    page.locator("[data-website-card-container]").first(),
  ).toHaveScreenshot("layout-page.png");
});
