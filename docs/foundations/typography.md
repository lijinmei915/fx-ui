...
来源：企业 Figma **web 字体规范**（fx-ui 是 web 库，以 web 规范为准；移动端字号另有一套，见 DEC-004）。FDS 统一定义字号、行高、字重和文本角色；不同技术框架通过适配层消费同一套 Token，业务页面不直接写死物理数值。

| 当前 Web 适配器调用 | 当前映射 | 字号/行高 | 场景 |
|------|------|------|------|
...
**实现方式**：
- 业务先按用途选择 FDS 文本角色，不直接选择 Primitive 字号或写死数值。
- 当前 React 适配器将文本角色映射为 `text-*` 调用；Vue 2 等其他框架应通过各自适配层引用相同 Token。
- 主题面板继续只改底层 Token，不改组件调用代码。
- Web 正文与操作文字底线是 **12px**；`11px` 仅用于 BI 图表中空间受限的极小刻度或数据标注，不用于正文、按钮、标签或关键状态。需要弱化普通界面文字时优先使用颜色、字重、透明层级或空间关系。

**完整 Primitive 字号阶**：`11 / 12 / 13 / 14 / 15 / 16 / 18 / 20 / 22 / 24 / 28 / 32 / 40px`。常规页面优先消费下表的语义字号；其余档位用于密度预设、展示标题与 BI 场景，不由业务页面直接引用 Primitive。

**行高计算规则**：先按用途选择倍率区间，再将结果吸附到最接近的偶数像素行高档。`11–16px` 的小字与正文优先使用约 `1.4–1.5`，`18–22px` 的界面标题优先使用约 `1.4–1.55`，`24–40px` 的展示标题收紧到约 `1.2–1.35`。最终配对还要兼顾中文上下留白和既有界面密度，因此是受治理的离散结果，不是运行时公式。当前边界档为 `11/16`（约 1.45，仅 BI 极小标注）和 `40/48`（1.2，单行大数据或展示标题）。

**字号 + 行高**（默认正文 = 16）：

| 当前 Web 适配器调用 | 字号/行高 | 字重 | 层级/场景 |
|------|-----|------|------|
...
> Figma 中的命名只作为来源参考；FDS 已将完整 Web 字号阶收口为 Primitive，并由统一语义角色和密度预设消费，不再维护中英文两套平行变量名。
