...
export type CrmAppShellFrame = "inset" | "continuous";
...
function CrmAppShell({
  ...
  children: ReactNode;
  height?: number;
  /** Inset keeps the CRM content as a distinct work surface; continuous removes that inner frame. */
  frame?: CrmAppShellFrame;
  navigation?: boolean;
  renderChrome?: (slot: "topbar" | "navigation", content: ReactNode) => ReactNode;
  topBar?: boolean;
}) {
  ...
  const topBarContent = <TopBar>
    ...
    <DropdownMenu>
      <DropdownMenuTrigger nativeButton={false} render={<Avatar className="size-8 cursor-pointer outline-none transition-opacity hover:opacity-90 focus-visible:ring-3 focus-visible:ring-ring/50"><AvatarImage src="/avatars/01.jpg" alt="李明" /><AvatarFallback colorful>李</AvatarFallback></Avatar>} />
      <DropdownMenuContent align="end" className="w-44"><DropdownMenuItem>个人中心</DropdownMenuItem><DropdownMenuItem>账号设置</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem variant="destructive">退出登录</DropdownMenuItem></DropdownMenuContent>
    </DropdownMenu>
  </TopBar>;
  return (
    <TooltipProvider>
      <WebsiteCardContainer padding="none" shape="square" tone="page" data-slot="crm-app-shell" data-frame={frame}>
        {topBar && (renderChrome?.("topbar", topBarContent) ?? topBarContent)}

        {/* 灰底 + 圆角白卡浮起：外留白对齐公司 Figma —— 顶/左(导航↔卡)/右 各 12，底 0；导航与内容卡间隔 12 */}
        <div className="flex min-h-0 gap-3 pt-3 pr-3" style={{ height }}>
          {navigation && (renderChrome?.("navigation", <CrmShellNav />) ?? <CrmShellNav />)}
          <div data-slot="crm-app-shell-content" className={frame === "inset" ? "flex min-w-0 flex-1 flex-col overflow-hidden rounded-lg bg-card" : "flex min-w-0 flex-1 flex-col overflow-hidden bg-transparent"}>
            {children}
          </div>
        </div>
