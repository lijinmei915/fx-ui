...
export function TokensRadiusPage({ actions, lang, radiusTokens }: Props) {
  return <div className={docsSpacing.pageStack}>
    <section id="tokens-radius" className="flex flex-col gap-2"><PageLead crumb={lang === "en" ? "Design Tokens / Radius" : "设计令牌 / 圆角"} title={lang === "en" ? "Radius" : "圆角"} lead={lang === "en" ? "Radius tokens keep controls, cards, and overlays visually consistent across framework implementations." : "圆角 Token 用来统一不同框架中控件、卡片和浮层容器的视觉性格。"} actions={actions} /></section>
