...
export function GettingStartedPageAdapter({ actions, page, lang }: { actions: ReactNode; page: GettingStartedSlug; lang: Lang }) {
  return <GettingStartedPage
    ...
    renderGovernanceMap={(mapActions, mapLang, mapPage) => <GovernanceMapPage
      header={<><PageLead crumb="维护 / 现状" title={mapLang === "en" ? "Status" : "现状看板"} lead={mapLang === "en" ? "A developer-facing snapshot of fx-ui governance: what is already protected, what is still being structured, and which checks are the current gate." : "给开发者看的仓库现状快照：哪些规则已经被检查保护，哪些还在结构化，当前交付门禁是什么。"} actions={mapActions} /><GovernanceQuickLinks currentPage={mapPage} lang={mapLang} items={governanceQuickLinks} /></>}
