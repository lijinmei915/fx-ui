...
页面类型的视觉角色统一查 `docs/data/page-semantics.manifest.json`。基础 token 只提供无语义的视觉刻度，全局 semantic token 提供跨页面共用含义，页面类型只负责声明区域角色和组合意图。

装配页面时按以下顺序执行：

1. 从 `page-build-kit.manifest.json` 选择 `ready` archetype；如果页面类型是 `planned`，先沉淀真实 Block，不得临时拼页面。
2. 为页面区域选择已有页面角色，例如列表页使用 `filter-surface`、`table-header`，表单页使用 `form-surface`、`validation-error`。
3. 角色只能解析到全局 semantic token，不在页面调用处新增颜色、字号、间距、圆角或阴影值。
4. 角色映射变化先更新 `page-semantics.manifest.json`，再更新 Block/模板引用，最后运行 `npm run check:page-semantics` 与 `npm run test:visual`。

当前 `ready` 页面类型：列表页、详情页、编辑表单页、搭建器。Dashboard、认证页、设置页已登记为 `planned`，待真实 Block 和视觉证据完成后再开放。
...
`#page-builder` 是顶栏“页面”旁的一级独立工作台，路由使用 `workspace` 外壳，不进入文档侧栏，也不渲染 PageLead、目录或文档内容限宽。标题旁的受控下拉只切换“基础组件搭建 / 业务组件搭建 / 页面搭建”：基础组件搭建从默认容器开始，通过两列可视素材块点击或拖入容器、文字、图标、分割线与结构预设，再在右侧配置布局、颜色、间距和交互状态；实时画布就是发布前预览，不再重复设置“生成候选 / 预览验收”步骤。页面模式组合已验证模板和 Block；业务组件模式从空白画布开始，通过搜索加入白名单组件并组织结构。三者都不是自由样式编辑器，不接受 JSX、CSS class 或任意样式值。统一真相源为 `docs/data/page-builder.manifest.json`。
...
基础组件搭建始终在当前工作台实时渲染受控草稿，并固定采用单根容器模型：第一个容器是默认白底的最外层容器，后续新增容器只能成为它的直接子项，不生成第二个根节点或更深的容器嵌套。全局只负责命名与发布，方向、子项间距、内边距和尺寸在选中容器后配置；组件外边距由将来使用它的父级控制，不作为基础组件自身属性。基础组件属性页只保留当前资产的属性标题和真实控件，不重复显示 Token 标签、选中说明和基础资产来源；这些治理元信息仍保留在业务组件属性页。用户完成命名并选择“新建组件”或“更新已有组件”后，可直接发布当前画布；发布只提交到对应入库或更新队列，不直接覆盖 `src/components/ui/*`。组件 API、token、交互/无障碍与视觉检查仍由后续 Playground 与入库治理链执行，删除重复确认门不等于绕过组件治理。
