...
function projectComponent(item, detail) {
  ...
  if (detail === "source") return { ...item, apiSource: item.apiSource, sourceReadRequired: true }
  return brief
}
...
function plan(query, options) {
  ...
  print({
    ...
    sourcePointers: [
      ...
      { file: "src/App.tsx", symbol: "CustomerListTemplate", purpose: "Existing complete list-page reference." },
    ],
    workflow: [
...
switch (command) {
  ...
  case "build": {
    const item = buildKit.archetypes.find((archetype) => archetype.id === args[0])
