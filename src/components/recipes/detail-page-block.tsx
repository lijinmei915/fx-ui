...
export function DetailPageBlock({
  ...
  return (
    <div data-slot="detail-page-block" className="flex flex-col gap-(--fx-panel-gap)">
      <WebsiteCardContainer data-slot="detail-page-header">
        <CardHeader>
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-(--fx-control-gap)">{onEdit ? <Button onClick={onEdit}>{editLabel}</Button> : null}{actions}</div>
          </div>
        </CardHeader>
      ...
      {tabbed ? <WebsiteCardContainer data-slot="detail-page-tabs" padding="none"><Tabs defaultValue={tabs[0].id} className="p-(--fx-panel-padding)"><TabsList>{tabs.map((tab) => <TabsTrigger key={tab.id} value={tab.id}>{tab.label}</TabsTrigger>)}</TabsList>{tabs.map((tab) => <TabsContent key={tab.id} value={tab.id} className="pt-(--fx-panel-gap)">{tab.content}</TabsContent>)}</Tabs></WebsiteCardContainer> : null}

      <div className="grid gap-(--fx-panel-gap) lg:grid-cols-2">
