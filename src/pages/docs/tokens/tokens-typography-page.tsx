...
{ name: "--fds-g-font-size-11", value: "11px / 16px", cls: "text-[11px]", usage: "BI 图表极小标注；不用于正文和操作文字", usageEn: "Tiny BI chart labels; not for body or action text" },
{ name: "--fds-g-font-size-12", value: "12px / 18px", cls: "text-xs", usage: "最小辅助信息、紧凑场景", usageEn: "Small helper text and compact contexts" },
{ name: "--fds-g-font-size-13", value: "13px / 18px", cls: "text-[13px]", usage: "28px 紧凑控件内部文字", usageEn: "Text inside compact 28px controls" },
{ name: "--fds-g-font-size-14", value: "14px / 20px", cls: "text-sm", usage: "字段标签、按钮、菜单项、辅助信息", usageEn: "Labels, buttons, menus, helper text" },
{ name: "--fds-g-font-size-15", value: "15px / 22px", cls: "text-[15px]", usage: "舒展密度下的紧凑控件文字", usageEn: "Compact control text in spacious density" },
{ name: "--fds-g-font-size-16", value: "16px / 24px", cls: "text-base", usage: "默认正文、菜单、列表和表单", usageEn: "Default body, menus, lists, and forms" },
{ name: "--fds-g-font-size-18", value: "18px / 28px", cls: "text-lg", usage: "模块、卡片和组件标题", usageEn: "Module, card, and component titles" },
{ name: "--fds-g-font-size-20", value: "20px / 30px", cls: "text-xl", usage: "详情页标题", usageEn: "Detail page titles" },
{ name: "--fds-g-font-size-22", value: "22px / 32px", cls: "text-[22px]", usage: "舒展密度下的层级标题", usageEn: "Hierarchy titles in spacious density" },
{ name: "--fds-g-font-size-24", value: "24px / 32px", cls: "text-2xl", usage: "小型展示标题", usageEn: "Small display titles" },
{ name: "--fds-g-font-size-28", value: "28px / 36px", cls: "text-[28px]", usage: "页面与中型展示标题", usageEn: "Page and medium display titles" },
{ name: "--fds-g-font-size-32", value: "32px / 40px", cls: "text-[32px]", usage: "大型展示标题", usageEn: "Large display titles" },
{ name: "--fds-g-font-size-40", value: "40px / 48px", cls: "text-[40px]", usage: "超大数据与展示标题", usageEn: "Hero metrics and extra-large display titles" }];
...
export function TokensTypographyPage({ actions, lang, roles, conventions, sizeTokens, weightTokens, familyTokens }: Props) {
  ...
  return (
    <div className={docsSpacing.pageStack}>
      <section id="tokens-typography" className="flex flex-col gap-2"><PageLead crumb={lang === "en" ? "Design Tokens / Typography" : "设计令牌 / 排版"} title={lang === "en" ? "Typography" : "排版"} lead={lang === "en" ? "Typography starts with size, line height, weight, and family, then combines them into text roles." : "排版先定义字号与行高、字重和字体，再组合成可直接使用的文本角色。"} actions={actions} /></section>
      ...
      <section id="tokens-typography-roles" className={`${docsSpacing.sectionStack} scroll-mt-24`}>
        <div><h2 className="text-xl font-bold tracking-tight">{lang === "en" ? "Text roles 文本角色" : "文本角色"}</h2><p className="mt-1 text-sm text-muted-foreground">{lang === "en" ? "Choose by purpose first. Call the role utility; its size and weight remain traceable base tokens." : "基础字号、字重与字族在这里组合为可直接调用的文本角色。"}</p></div>
